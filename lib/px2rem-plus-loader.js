...
module.exports = function (source) {
    ...
    if (query.path && Array.isArray(query.path) && query.path.length) { // 判断是否有path参数
        var flag = true // 判断路径是否匹配
        for (var i = 0; i < query.path; i++) { // 循环判断是否匹配
            if (!pathReg(query.path[i], shortFilePath)) {
                flag = false
                break
            }
        }
